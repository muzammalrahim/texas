...
def update_all_data(excel_file_path):
    """
    #Getting all data from scraper and upload it to DB.
    #Parameter : excel_file_path is location of file contains attorney's name and bar card number.
    #Return is True when successfully run.
    :param excel_file_path:
    :return: True
    """
    def process_attorney_name(name):
        """
        # Split first name and last name from fullname.
        # Parameter : name is full name (ex: Ms. April Elaine Smith).
        # Return is first name and last name of attorney.
        :param name:
        :return: first name and last name of attorney
        """
        name = str(name)
        name = name.replace('  ', ' ')
        if len(name.split()) == 2:
        ...
        if 'Jr'.upper() in name.split()[-1].upper():
            final = name.split()[-2]
        elif 'II'.upper() in name.split()[-1].upper():
            final = name.split()[-2]
        elif 'IV'.upper() in name.split()[-1].upper():
            final = name.split()[-2]
        else:
            final = name.split()[-1]
        return name.split()[0] + ' ' + final
    ...
    # Read Attorneys from Excel Sheet.
    attorneys = []
    bar_card_numbers = []
    ...
    # Get all Links of Attorneys without Bar Card Number in Excel Sheet.
    simples = []
    pool = ThreadPool(32)
    ...
    # Update new Attorneys.
    # This section will be run for attorneys don't have bar card name.
    # Result is array contains link of court.
    ...
    for _, row in enumerate(pool.imap_unordered(get_texasbar_details, simples)):
        # In this section script will run to save new attorney to DB.
        # If attorney is in DB, will append only bar card number to get updated case.
        # Parameter: simples contains attorney names and links.
    ...
    for r, chunk in enumerate(pool.imap_unordered(get_cases, bar_card_numbers)):
        # Get New cases about all attorneys using bar card number.
        # Then will append it all_case array
        try:
    ...
    for r, c in enumerate(pool.imap_unordered(get_case, all_case)):
        # Get New case data about all cases.
        # Then will save case to Case table.
    ...
    # Write all lawyers to lawyers.txt to show in Search Attorneys page.
    ...
    # Write all case_type to case_types.txt to show in All Cases Types page.
    ...
    # Write all text of attorneys to show result in web page
    for case in cases:
...
def add_lawyer(request):
    """
    # Add new lawyers from excel file or link.
    :param request:
    :return:
    """
